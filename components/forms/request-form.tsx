...
import type { HRRequest, CreateRequestData, UpdateRequestData, RequestType } from "@/lib/types"

const requestSchema = z.object({
  FUNCIONARIO_ID: z.string().min(1, "Funcionário é obrigatório"),
  TIPO: z.enum(["DOCUMENTO", "EQUIPAMENTO", "BENEFICIO", "TREINAMENTO", "OUTROS"]),
  DESCRICAO: z.string().min(1, "Descrição é obrigatória").max(500, "Máximo de 500 caracteres"),
...
export function RequestForm({ request, onSubmit, isSubmitting, onCancel }: RequestFormProps) {
  ...
  const {
    ...
    defaultValues: {
      ...
      DATA_SOLICITACAO: request?.DATA_SOLICITACAO?.split("T")[0] || new Date().toISOString().split("T")[0],
      ...
      <FieldGroup>
        ...
        <div className="grid grid-cols-2 gap-4">
          <Field>
            ...
            {errors.TIPO && <FieldMessage variant="error">{errors.TIPO.message}</FieldMessage>}
          </Field>

          <Field>
            <FieldLabel htmlFor="DATA_SOLICITACAO">Data de Solicitação *</FieldLabel>
            <Input id="DATA_SOLICITACAO" type="date" {...register("DATA_SOLICITACAO")} />
            {errors.DATA_SOLICITACAO && <FieldMessage variant="error">{errors.DATA_SOLICITACAO.message}</FieldMessage>}
          </Field>
        </div>
        ...
        <Field>
          ...
          />
          {errors.DESCRICAO && <FieldMessage variant="error">{errors.DESCRICAO.message}</FieldMessage>}
        </Field>
        ...
        {request && (
          <div className="grid grid-cols-2 gap-4">
            <Field>
              ...
              <Input id="DATA_RESPOSTA" type="date" {...register("DATA_RESPOSTA")} />
            </Field>
